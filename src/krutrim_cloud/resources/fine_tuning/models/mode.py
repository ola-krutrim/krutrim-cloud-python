...
from ....types.fine_tuning.models.mode_retrieve_response import ModeRetrieveResponse
...
class ModeResource(SyncAPIResource):
    ...
    def _validate_retrieve_parameter(self, mode, engine):
        # Validate mode
    ...
    def retrieve(
        ...
        *,
        ...
        """
        self._validate_retrieve_parameter(mode=mode, engine=engine)
        if not engine:
        ...
        return self._get(
            f"/api/v1/fine_tuning/models/{engine}/{mode}",
    ...
    async def retrieve(
        ...
        *,
        ...
        Args:
          ...
          timeout: Override the client-level default timeout for this request, in seconds
        """
        await self._validate_retrieve_parameters(mode=mode, engine=engine)
        if not engine:
            raise ValueError(f"Expected a non-empty value for `engine` but received {engine!r}")
        if not mode:
            raise ValueError(f"Expected a non-empty value for `mode` but received {mode!r}")
